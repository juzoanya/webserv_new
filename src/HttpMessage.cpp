...
/*   Updated: 2024/02/20 00:03:24 by juzoanya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    // std::cout << "parse Body - maxBodySize: " << _maxBodySize << std::endl;
...
void    HttpMessage::setCgiResponseDone( void )
{
    ...
    if (!header.hasHeader("connection")) {
        ...
        // _buffer2 += "connection: close" + ws_http::crlf;
    }
...
int     HttpMessage::readFromSocketAndParseHttp( int socketfd, int flags )
{
    ...
    std::cout << "\n-------- data read ---------\n" << std::string(readBuffer, readBufferSize) << "\n------------------------------" << std::endl;
...
int     HttpMessage::sendDataToSocket( int socketfd, int flags )
{
    if (_contentLength == 0)
        return (0);
    // // std::cout << "\n-------- data to send --------\n" << std::string(_dataPtr, _contentLength) << "\n------------------------------" << std::endl;
