...
/*   Updated: 2024/02/03 16:14:32 by mberline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ws_http::statuscodes_t    HttpStatic::handleData( int socketfd, buff_t::const_iterator it_start, buff_t::const_iterator it_end )
{
...
void    HttpStatic::setCgiData( void )
{
    std::vector<std::string>    cgiEnv;

    http_header_map_t::iterator headerIt;
    cgiEnv.push_back(std::string("QUERY_STRING=") + this->_request.query);
    cgiEnv.push_back(std::string("REQUEST_METHOD=") + this->_request.method);
    headerIt = this->_request.headerMap.find("content-type");
    if (headerIt == this->_request.headerMap.end())
        cgiEnv.push_back(std::string("CONTENT_TYPE=") + headerIt->second);
    else
        cgiEnv.push_back(std::string("CONTENT_TYPE="));
    cgiEnv.push_back(std::string("CONTENT_LENGTH=") + toStr(this->_fileData.size()));
    cgiEnv.push_back(std::string("SCRIPT_FILE_NAME=") + this->_httpConfig.getFilePath());
    cgiEnv.push_back(std::string("SCRIPT_NAME=") + this->_request.pathDecoded);
    cgiEnv.push_back(std::string("PATH_INFO="));
    cgiEnv.push_back(std::string("PATH_TRANSLATED=") + this->_httpConfig.getRootPath());
    cgiEnv.push_back(std::string("REQUEST_URI=") + this->_request.fullUri);
    ...
    cgiEnv.push_back(std::string("REMOTE_ADDR=") + this->clientIp);
    cgiEnv.push_back(std::string("REMOTE_PORT=") + this->clientPort);
    cgiEnv.push_back(std::string("SERVER_NAME=") + this->_httpConfig.getServerName());
    cgiEnv.push_back(std::string("SERVER_ADDR=") + this->_configHandler->serverIp);
    cgiEnv.push_back(std::string("SERVER_PORT=") + this->_configHandler->serverIp);
