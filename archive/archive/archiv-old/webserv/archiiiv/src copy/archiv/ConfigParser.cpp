...
void	ConfigParser::parseDirective(std::string& directive, std::map<std::string, std::vector<std::string> >& contextMap)
{
	...
	line >> key;
	if (key == "listen") {
		std::string listenpair;
		line >> listenpair;
		std::size_t pos = listenpair.find(':');
		if (pos != std::string::npos) {
			value.push_back(listenpair.substr(0, pos));
			value.push_back(listenpair.substr(pos + 1, std::string::npos));
		}
	} else {
		while (!line.eof())
		{
...
void	ConfigParser::configParser(char *file)
{
	...
	for (int i = 0; i != this->getServerCount(); ++i) {
		...
		for (std::size_t j = 0; j != currServerConfig.locationConfig.size(); ++j) {
			printConfigMap("\tLocation Config: " + currServerConfig.locationConfig[j].first + ": ", currServerConfig.locationConfig[j].second);
		}
	}
}

void	ConfigParser::setPredefConfig( void )
{
	// this.setSe
}
