...
/*   Updated: 2024/02/08 19:36:53 by mberline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
HttpHandler::HttpHandler( ConfigHandler& configHandler, WsIpPort const & ipPort )
 : ipPortData(ipPort),  _request(&configHandler, true), _configHandler(&configHandler), _cgiProcess(NULL)
//  : ipPortData(ipPort),  _request(&configHandler, true), _configHandler(&configHandler), _cgiProcess(NULL), _intRedirectNbr(0)
{ }
...
ws_http::statuscodes_t    HttpHandler::handleData( buff_t::const_iterator it_start, buff_t::const_iterator it_end )
{
    ...
    if (status == ws_http::STATUS_200_OK)
        ...
        status = this->httpContent.setResponseDefaultError(this->_request.status);
...
//     this->_httpConfig = this->_configHandler->getHttpConfig(requestPathDecoded, this->_request.hostHeader);
...
//     std::cout << "filePath: " << this->_httpConfig.getFilePath() << std::endl;
...
//         this->_cgiProcess = new HttpHandler::CgiHandler(*this, config, this->_request, this->_fileData, this->_headerBuffer);
//         return (ws_http::STATUS_1001_INTERNAL_PROCESS_CGI);
//     }

//     std::cout << "FILEPLATH: " << this->_httpConfig.getFilePath() << std::endl;
                                                            ...
                                                            std::string const & method, std::string const & requestPathDecoded, int redirNo )
{
   std::cout << "FILEPLATH: " << config.getFilePath() << std::endl;
    FileInfos file(config.getFilePath(), this->_request.pathDecoded);
    if (file.getError() == 0 && S_ISREG(file.getMode())) {
        std::cout << "----> is a regular file" << std::endl;
        std::ifstream   ifs(config.getFilePath().c_str(), std::ios::binary | std::ios::ate);
        if (ifs.is_open()) {
            this->httpContent.setResponseStatic(statusCode, ifs, config.getMimeType());
            ifs.close();
            return (statusCode);
        }
    } else if (file.getError() == 0 && S_ISDIR(file.getMode())) {
        ...
        std::stringstream ss;
        file.setDirListing(ss);
        return (this->httpContent.setResponseStatic(statusCode, ss, "text/html"));
    } else if (file.getError() == EACCES) {
        std::cout << "----> permission denied" << std::endl;
        return (processResponse(ws_http::STATUS_403_FORBIDDEN, "GET", config.getErrorPage("403"), ++redirNo));
    }
...
ws_http::statuscodes_t    HttpHandler::processResponse( ws_http::statuscodes_t statusCode, std::string const & method, std::string const & requestPathDecoded, int redirNo )
{
    ...
    if (statusCode >= ws_http::STATUS_400_BAD_REQUEST && redirNo > 0)
        return (this->httpContent.setResponseDefaultError(statusCode));
    if (redirNo >= 10)
        return (this->httpContent.setResponseDefaultError(ws_http::STATUS_500_INTERNAL_SERVER_ERROR));
    ...
    if (redirect.redirectStatus != ws_http::STATUS_UNDEFINED)
        return (this->httpContent.setResponseRedirection(redirect.redirectStatus, redirect.location));
    ...
    processResponseFile(config, statusCode, method, requestPathDecoded, redirNo);
...
// HttpHandler::CgiHandler::CgiHandler( WsIpPort const & clientIpPort, WsIpPort const & serverIpPort, HttpConfig const & config, HttpRequest const & request, buff_t & resultBodyBuffer, buff_t & resultHeaderBuffer )
HttpHandler::CgiHandler::CgiHandler( WsIpPort const & clientIpPort, WsIpPort const & serverIpPort, HttpConfig const & config, HttpRequest const & request, HttpContent & clientResponseBuffer)
 : _requestHelper(NULL, false), _clientResponseBuffer(clientResponseBuffer)
//  : _requestHelper(NULL, false), _resultHeaderBuffer(resultHeaderBuffer), _resultBodyBuffer(resultBodyBuffer)
{
...
ws_http::statuscodes_t    HttpHandler::CgiHandler::handleData( buff_t::const_iterator it_start, buff_t::const_iterator it_end )
{
    ...
    if (it_start == it_end) {
        ...
        if (itStatus == this->_requestHelper.headerMap.end() || itContType == this->_requestHelper.headerMap.end())
            return (this->_clientResponseBuffer.setResponseDefaultError(ws_http::STATUS_500_INTERNAL_SERVER_ERROR));
        ss << "HTTP/1.1 " << itStatus->second << ws_http::crlf;
        ss << "connection: close" << ws_http::crlf;
        ...
        this->_httpHandler.httpContent = HttpContent(&this->_resultHeaderBuffer, &this->_resultBodyBuffer);
    ...
    if (this->_requestHelper.status == ws_http::STATUS_200_OK) {
        ...
        if (this->_requestHelper.status == ws_http::STATUS_200_OK)
            this->_clientResponseBuffer.appendContentByBuffer(this->_requestHelper.endPos, it_end, false);
