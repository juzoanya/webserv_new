...
/*   Updated: 2024/02/08 18:24:41 by mberline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
HttpConfig::HttpConfig( ws_config_t* serverDirectives, ws_config_t* locationDirectives, std::string const & pathDecoded, std::string const & serverName )
 : _serverDirectives(serverDirectives), _locationDirectives(locationDirectives), _serverName(serverName)
{
    ...
    if (locationDirectives) {
        ...
        ws_config_t::iterator itLoc = locationDirectives->find("location");
        ...
        if (it != locationDirectives->end() && it->second.size() == 1) {
            this->_rootPath = it->second[0];
            this->_filePath = this->getRootPath() + pathDecoded.substr(location.size(), std::string::npos);
            // this->_filePath = it->second[0] + pathDecoded.substr(location.size(), std::string::npos);
    ...
    if (serverDirectives) {
        ...
        if (it != serverDirectives->end()) {
            this->_rootPath = it->second[0];
            this->_filePath = this->getRootPath() + pathDecoded;
            std::cout << "rooooot: " << this->getRootPath() << std::endl;
            // this->_filePath = it->second[0] + pathDecoded;
...
std::string const & HttpConfig::getCgiExecutable( void ) const
{
    ...
    if (value.first == std::string("cgi") + fileExtension && value.second.size() == 1)
        return (value.second[0]);
    return (this->_dummyMapVal.first);
}
