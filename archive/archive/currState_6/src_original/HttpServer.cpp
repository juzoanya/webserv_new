...
void	HttpServ::startMonitoring( int fd, short events, HttpHandler* data )
{
...
int	HttpServ::handleAccept( void )
{
	while (true) {
		...
		this->startMonitoring(clientSocket, POLLIN | POLLOUT, new HttpHandler(this->serverConfigs, ipPortData));
...
void	HttpServ::handleRead( std::size_t index )
{
	...
	if (readByte == -1) {
		...
		this->_clients[index]->handleData(this->_readBuffer.begin(), this->_readBuffer.begin() + readByte);
	}
}
...
void	HttpServ::handleWrite( std::size_t index )
{
	if ( this->_clients[index]->dataSize == 0)
		return ;
	int sendBytes = send(this->_pollFds[index].fd, this->_clients[index]->dataPtr, this->_clients[index]->dataSize, 0);
	if (sendBytes == -1) {
		stopMonitoring(index);
		...
		this->_clients[index]->dataPtr += sendBytes;
		this->_clients[index]->dataSize -= sendBytes;
	}
}
...
void	HttpServ::runPoll( void )
{
	...
	for (std::size_t i = 1; i < currSize; ++i)
	{
		if (this->_pollFds[i].revents == 0 || this->_pollFds[i].fd == -1)
			continue;
		if (this->_pollFds[i].revents & (POLLIN | POLLHUP)) {
