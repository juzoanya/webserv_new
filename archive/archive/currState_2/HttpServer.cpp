...
/*   Updated: 2024/01/29 09:16:19 by mberline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	HttpServer::stopMonitoring(int socketFd, int clientIndex)
{
...
void	HttpServer::handleRead(int socketFd, int clientIndex)
{
	buff_t	buffer(4096);
...
void	HttpServer::handleWrite(int socketFd, int clientIndex)
{
	if (!this->_clients[clientIndex]) {
		std::cout << "no HttpStatic instance for this ClientSocket in the Map\n";
		return ;
