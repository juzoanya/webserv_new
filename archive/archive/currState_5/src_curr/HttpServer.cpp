...
SocketData::SocketData( bool isclient, int index ) : isClient(isclient), arrIndex(index), httpMessage(NULL)
{
	// if (!)
}
...
// void	HttpServ::addMonitoring( int fd, short events, HttpMessage* data )
// {
...
void    HttpServ::addMonitoring( int fd, short events, bool isClient )
{
	std::size_t i = 0;
	while (i != this->_pollFds.size() && this->_pollFds[i].fd != -1)
		i++;
	if (i == this->_pollFds.size()) {
		this->_pollFds.push_back(pollfd());
		this->_data.push_back(SocketData(isClient, i));
	} else {
...
int	HttpServ::handleAccept( void )
{
	while (true) {
		...
		this->addMonitoring(clientSocket, POLLIN | POLLOUT, new HttpMessage(&this->serverConfigs));
		std::cout << "[" << ipPortData.getIpStr() << ":" << ipPortData.getPortStr() << "]: new clientsocket: " << clientSocket << std::endl;
	}
	return (0);
}

ws_http::statuscodes_t  HttpServ::processResponse( ws_http::statuscodes_t statusCode, std::string const & method, std::string const & requestPathDecoded, bool internal )
{
...
void	HttpServ::handleRead( std::size_t index )
{
	...
	if (readByte == -1) {
		...
		ws_http::statuscodes_t status = this->_clients[index]->handleDataFromSocket(this->_readBuffer.begin(), this->_readBuffer.end());
		if (status == ws_http::STATUS_200_OK)
			this->processResponse(status, this->_clients[index]->method, this->_clients[index]->pathDecoded, false);
	}
}
...
void	HttpServ::runPoll( void )
{
	...
	for (std::size_t i = 1; i < currSize; ++i)
	{
		...
		if (this->_pollFds[i].revents & POLLOUT && this->_clients[i]) {
			int sendres = this->_clients[i]->writeToSocket(this->_pollFds[i].fd);
			if (sendres == -1) {
				stopMonitoring(i);
