...
/*   Updated: 2024/02/07 18:01:59 by mberline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	HttpServ::startMonitoring( int fd, short events, AHttpWrapper* data )
{
	std::size_t i = 0;
...
void	HttpServ::stopMonitoring( AHttpWrapper const & instance )
{
	std::cout << "stop monitoring at intex: " << index << " | pollFd vec size: " << this->_pollFds.size() << " | clients vec size: " << this->_clients.size() << std::endl;
	close(this->_pollFds[instance.indexNbr].fd);
	this->_pollFds[instance.indexNbr].fd = -1;
	if (this->_clients[instance.indexNbr]) {
		delete this->_clients[instance.indexNbr];
		this->_clients[instance.indexNbr] = NULL;
	}
}
...
int	HttpServ::handleAccept( void )
{
	while (true) {
		...
		// this->addMonitoring(clientSocket, POLLIN | POLLOUT, new HttpWrapper(serverConfigs, ipPortData));
		std::cout << "[" << ipPortData.getIpStr() << ":" << ipPortData.getPortStr() << "]: new clientsocket: " << clientSocket << std::endl;
	}
	return (0);
}

// ws_http::statuscodes_t    HttpServ::processResponse( HttpWrapper& httpWrapper, ws_http::statuscodes_t statusCode, std::string const & method, std::string const & requestPathDecoded, int redirNo )
// {
//     std::cout << "\n---------------- HttpHandler::processResponse ----------------\n"
//         << "status: " << ws_http::statuscodes.at(statusCode)
//         << " | method: " << method << " | requestPathDecoded: "
//         << requestPathDecoded << " | redirNo: " << redirNo << std::endl;
// 	if (statusCode >= ws_http::STATUS_400_BAD_REQUEST && redirNo > 0)
// 		return (httpWrapper.content.setDefaultError(statusCode));
// 	if (redirNo >= 10)
// 		return (httpWrapper.content.setDefaultError(ws_http::STATUS_500_INTERNAL_SERVER_ERROR));

//     HttpConfig config = this->serverConfigs.getHttpConfig(requestPathDecoded, httpWrapper.request.hostHeader);
    
//     Redirect redirect = config.getRedirection();
//     if (redirect.redirectStatus != ws_http::STATUS_UNDEFINED)
//         return (httpWrapper.content.setContent(redirect.redirectStatus, NULL, "", redirect.location));

//     if (!config.checkAllowedMethod(method))
//         return (this->processResponse(httpWrapper, ws_http::STATUS_405_METHOD_NOT_ALLOWED, "GET", config.getErrorPage("405"), ++redirNo));

//     std::string const & cgiExecutablePath = config.getCgiExecutable();
//     if (!cgiExecutablePath.empty())
//         return (ws_http::STATUS_UNDEFINED);

//     FileInfos file(config.getFilePath(), requestPathDecoded);
...
//         if (ifs.is_open()) {
//             httpWrapper.content.setContent(statusCode, &ifs, config.getMimeType());
//             ifs.close();
//             return (statusCode);
//         }
...
//             return (this->processResponse(httpWrapper, statusCode, method, requestPathDecoded + "/" + indexFileName, ++redirNo));
//         if (!config.hasDirectoryListing())
// 			return (this->processResponse(httpWrapper, ws_http::STATUS_403_FORBIDDEN, "GET", config.getErrorPage("403"), ++redirNo));
//         std::stringstream ss;
//         file.setDirListing(ss);
//         return (httpWrapper.content.setContent(statusCode, &ss, "text/html"));
//     } else if (file.getError() == EACCES) {
//         std::cout << "----> permission denied" << std::endl;
// 		return (this->processResponse(httpWrapper, ws_http::STATUS_403_FORBIDDEN, "GET", config.getErrorPage("403"), ++redirNo));
//     }
//     std::cout << "----> not found" << std::endl;
// 	return (this->processResponse(httpWrapper, ws_http::STATUS_404_NOT_FOUND, "GET", config.getErrorPage("404"), ++redirNo));
// }

ws_http::statuscodes_t    HttpServ::processResponse( HttpWrapper& httpWrapper, ws_http::statuscodes_t statusCode, std::string const & method, std::string const & requestPathDecoded, int redirNo )
{
	std::multimap<int, int, int, int> jf;
	std::multimap<int, int, int, int>::iterator ev;
	std::map<int, int>::iterator eff;
	ev->
    std::cout << "\n---------------- HttpHandler::processResponse ----------------\n"
        << "status: " << ws_http::statuscodes.at(statusCode)
        << " | method: " << method << " | requestPathDecoded: "
        << requestPathDecoded << " | redirNo: " << redirNo << std::endl;
	if (statusCode >= ws_http::STATUS_400_BAD_REQUEST && redirNo > 0 || requestPathDecoded.empty())
		return (httpWrapper.content.setDefaultError(statusCode));
	if (redirNo >= 10)
		return (httpWrapper.content.setDefaultError(ws_http::STATUS_500_INTERNAL_SERVER_ERROR));
    ...
    Redirect redirect = config.getRedirection();
    if (redirect.redirectStatus != ws_http::STATUS_UNDEFINED)
		return (httpWrapper.content.setRedirection(redirect.redirectStatus, redirect.location));
...
void	HttpServ::handleRead( AHttpWrapper const & instance )
{
	ssize_t readByte = recv(instance.socketfd, this->_readBuffer.data(), this->_readBuffer.size(), 0);
	if (readByte == -1) {
		...
		if (instance.isCgi) {
			...
			// if (status == ws_http::STATUS_UNDEFINED && dynamic_cast<HttpHandler*>(this->_clients[index]))

		} else {
			instance.request.parseRequest(this->_readBuffer.begin(), this->_readBuffer.begin() + readByte);
			...
			this->_request = HttpRequest(&this->serverConfigs, true);
			return (status);
		}
	}
}
...
void	HttpServ::runPoll( void )
{
	...
	for (std::size_t i = 1; i < currSize; ++i)
	{
		...
		if (this->_pollFds[i].revents & POLLOUT && this->_clients[i] && !this->_clients[i]->empty()) {
			int sendres = this->_clients[i]->sendDataToSocket(this->_pollFds[i].fd, 0);
			if (sendres == -1) {
				stopMonitoring(i);
