...
/*   Updated: 2024/02/05 17:15:29 by mberline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
HttpContent::HttpContent( void )
 : headerPtr(NULL), headerSize(0), dataPtr(NULL), dataSize(0)
{ }
...
HttpContent::HttpContent( const buff_t* headerBuffer, const buff_t* bodyBuffer )
{
    ...
    this->dataPtr = bodyBuffer ? bodyBuffer->data() : NULL;
    this->dataSize = bodyBuffer ? bodyBuffer->size() : 0;
}
...
int HttpContent::sendDataToSocket( int socketfd, int flags )
{
    ...
    if (this->headerSize > 0) {
        ...
        writeByte = send(socketfd, this->dataPtr, this->dataSize, flags);
        if (writeByte > 0) {
            this->dataPtr += writeByte;
            this->dataSize -= writeByte;
        }
    } 
    return (writeByte);
}
...
{
    return (this->dataSize == 0 && this->headerSize == 0);
}



AHttpWrapper::AHttpWrapper( bool isCgiInstance )
 : indexNbr(0), socketfd(0), isCgi(isCgiInstance), headerPtr(NULL), headerSize(0), bodyPtr(NULL), bodySize(0)
{ }

AHttpWrapper::AHttpWrapper( const buff_t* headerBuffer, const buff_t* bodyBuffer, bool isCgiInstance, int index)
 : indexNbr(index), socketfd(0), isCgi(isCgiInstance)
{
    this->headerPtr = headerBuffer ? headerBuffer->data() : NULL;
    this->headerSize = headerBuffer ? headerBuffer->size() : 0;
...
AHttpWrapper::~AHttpWrapper( void )
{ }

int AHttpWrapper::sendDataToSocket( int socketfd, int flags )
{
    int writeByte = 0;
    if (this->headerSize > 0) {
        writeByte = send(socketfd, this->headerPtr, this->headerSize, flags);
...
bool    AHttpWrapper::empty( void ) const
{
    return (this->bodySize == 0 && this->headerSize == 0);
}

HttpWrapper::HttpWrapper( ConfigHandler& configHandler, WsIpPort ipPortData )
 : AHttpWrapper(false), ipPort(ipPortData), request(&configHandler, true)
{ }

HttpWrapper::~HttpWrapper( void )
{ }

ws_http::statuscodes_t HttpWrapper::setContent( ws_http::statuscodes_t status, std::istream& httpBodyStream, std::string const & mimeType )
{
    std::cout << "-- setHeader\n";
...
ws_http::statuscodes_t  HttpWrapper::setDefaultError( ws_http::statuscodes_t errorStatusCode )
{
    std::stringstream ss;
...
ws_http::statuscodes_t  HttpWrapper::setRedirection( ws_http::statuscodes_t status, std::string const & location )
{
...
AHttpHandler::AHttpHandler( bool isCgi ) : _isCgi(isCgi)
{ }

AHttpHandler::AHttpHandler( HttpContent const & data, bool isCgi ) : dataContent(data), _isCgi(isCgi)
{ }

AHttpHandler::~AHttpHandler( void )
{ }

bool    AHttpHandler::isCgi( void ) const
{
    return (this->_isCgi);
}

HttpHandler::HttpHandler( ConfigHandler& configHandler, WsIpPort const & ipPort )
 : AHttpHandler(HttpContent(NULL, NULL), false), ipPortData(ipPort),  _request(&configHandler, true), _configHandler(&configHandler), _intRedirectNbr(0)
{ }
...
HttpHandler::CgiHandler::CgiHandler( HttpHandler & httpHandler, HttpRequest & request, buff_t & resultBodyBuffer, buff_t & resultHeaderBuffer )
 : AHttpHandler(HttpContent(NULL, &request.requestBody.bodyBuffer), true), _requestHelper(NULL, false), _httpHandler(httpHandler), _resultHeaderBuffer(resultHeaderBuffer), _resultBodyBuffer(resultBodyBuffer)
{
