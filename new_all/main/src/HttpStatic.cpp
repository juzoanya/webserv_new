...
/*   Updated: 2024/01/22 20:56:36 by juzoanya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
HttpStatic::HttpStatic( ConfigHandler& configHandler, std::string const & clientIpStr, std::string const & clientPortStr )
 :  clientIp(clientIpStr), clientPort(clientPortStr),
 ...
 : fileName(name)
{
    ...
    std::ostringstream osFilseSize;
    osFilseSize << std::fixed << std::setprecision(1) << dSize << " " << suffixes[k];
    this->fileSize = osFilseSize.str();
